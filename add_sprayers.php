...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
 ...
 $stmt->bind_param("sssissiis", $category, $subcategory, $title, $stock, $address, $mobile, $acceptTerms,$user_id,$description);
    ...
    // Step 2: Insert unique attributes for Sprayers
    ...
    if (!$sprayers_stmt->execute()) {
        echo json_encode(array('status' => 'error', 'message' => 'Execute failed: ' . $sprayers_stmt->error));
        exit;
    }
    ...
    // Success response
    echo json_encode(array('status' => 'success', 'message' => 'Sprayers advertisement successfully submitted'));
    exit;
}
