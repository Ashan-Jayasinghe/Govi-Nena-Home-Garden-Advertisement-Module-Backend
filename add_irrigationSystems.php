...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
 ...
 $stmt->bind_param("sssissiis", $category, $subcategory, $title, $stock, $address, $mobile, $acceptTerms,$user_id,$description);
    ...
    // Step 2: Insert unique attributes for Irrigation Systems
    $condition = $_POST['condition'];
    $rentOrSell = $_POST['rentorsell'];
    $manufacturer = $_POST['manufacturer'];
    $price = $_POST['price'];

    $irrigation_stmt = $conn->prepare("INSERT INTO advertisement_irrigation_systems (advertisement_id, condition_, rent_or_sell, manufacturer, price) VALUES (?, ?, ?, ?, ?)");
    $irrigation_stmt->bind_param("isssd", $advertisement_id, $condition, $rentOrSell, $manufacturer, $price);

    if (!$irrigation_stmt->execute()) {
        echo json_encode(array('status' => 'error', 'message' => 'Execute failed: ' . $irrigation_stmt->error));
        exit;
    }
    ...
    // Success response
    echo json_encode(array('status' => 'success', 'message' => 'Irrigation Systems advertisement successfully submitted'));
    exit;
}
